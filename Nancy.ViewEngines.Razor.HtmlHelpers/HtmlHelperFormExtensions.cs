...
namespace Nancy.ViewEngines.Razor.HtmlHelpers
{
	...
	public static class HtmlHelperFormExtensions
	{
		...
		private static NonEncodedHtmlString GetFormTag(string action = null, string id = null, string name = null, string method = "POST")
		{
			var actionAttribute = action == null ? "" : string.Format(" action=\"{0}\"", action);
			var idAttribute = id == null ? string.Empty : string.Format(" id=\"{0}\"", id);
			if (name == null && id != null) name = id;
			var nameAttribute = name == null ? "" : string.Format(" name=\"{0}\"", name);
